...
def translate_file(target, file):
    """Translates text into the target language.
    Target must be an ISO 639-1 language code.
    See https://g.co/cloud/translate/v2/translate-reference#supported_languages
    """
    ...
    filex=translate_client.translate(filehandle.read())



    if isinstance(filex, six.binary_type):
        file = filex.decode('utf-8')


    # Text can also be a sequence of strings, in which case this method
    # will return a sequence of results for each text.
    result = translate_client.translate(
        filex, target_language=target)
...
if __name__ == '__main__':
    ...
    translate_file_parser = subparsers.add_parser(
        'translate-file', help=translate_file.__doc__)
    translate_file_parser.add_argument('target')
    translate_file_parser.add_argument('file')
    ...
    if args.command == 'detect-language':
        ...
        translate_file(args.target, args.file)
